...
Xidea 是一个面向持续学习任务的 **AI 学习编排系统**。我们想实现的不是另一个静态卡片工具，也不是一个泛化陪聊式 tutor，而是一套能持续理解学习目标与上下文，并决定“现在该怎么学”的系统。

今天用户可以分别用 GPT / Gemini 的学习模式做即时辅导，用 Anki 做科学复习，用语音对聊完成口语或陪练，但这些能力大多仍是分散的。Xidea 想做的不是再增加一个单点工具，而是把多模态学习、学习编排和科学复习放进同一套持续运转的学习系统里。

它的长期方向同时成立三件事：

- 支持多模态输入，而不是只接文本问答
- 支持多种学习形式，并由系统判断当前更合适的训练动作
- 具备独立的复习能力，用专门的 `Review Engine` 决定何时复习、复习什么

## 实现愿景

- 让系统围绕明确主题、材料、知识点和学习状态，持续组织学习过程，而不是停留在一次性的问答或内容展示
- 让即时学习、复习安排和长期记忆积累不再割裂，而是成为同一条持续学习链路
- 让知识点、学习过程、复习节奏和长期理解形成闭环，而不是散落在互不相关的工具与页面里
- 让输入形式和训练形式可以逐步扩展，但产品体验始终围绕“系统决定你现在该怎么学”展开

## 当前实现

- 当前仓库先围绕一个项目型学习场景落地最小实现：把主题、材料、知识点、学习过程和状态变化收进同一条主路径
- 当前实现优先验证一件事：**系统会根据学习状态安排下一步学习动作**
- 当前产品壳仍然比较克制，重点先放在把这条学习编排链路讲清楚并跑通

如果你想先看当前范围、进度和下一步，而不是启动项目，建议直接从 [docs/README.md](docs/README.md) 开始。
