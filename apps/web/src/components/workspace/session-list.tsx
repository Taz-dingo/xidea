...
const SESSION_LIST_COLLAPSED_MAX_HEIGHT = "max-h-[15.5rem]";
        ...
        <>
          {canExpand && isExpanded ? (
            ...
            <div className={canExpand ? `relative overflow-hidden ${SESSION_LIST_COLLAPSED_MAX_HEIGHT}` : undefined}>
              {cards}
              {canExpand ? (
                <div className="pointer-events-none absolute inset-x-0 bottom-0 h-14 bg-gradient-to-t from-[var(--xidea-white)] via-[var(--xidea-white)]/92 to-transparent" />
              ) : null}
            </div>
          )}

          {canExpand ? (
            <div className="flex justify-end border-t border-[var(--xidea-border)] pt-2">
              <Button
