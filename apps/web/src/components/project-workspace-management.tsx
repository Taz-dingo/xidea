...
interface KnowledgePointDraftValue {
...
export function CreateKnowledgePointPanel({
  draft,
  materials,
  onCancel,
  onChange,
  ...
  draft: KnowledgePointDraftValue;
  materials: ReadonlyArray<SourceAsset>;
  onCancel: () => void;
  onChange: (draft: KnowledgePointDraftValue) => void;
  onSave: () => void;
}): ReactElement {
  const isDisabled =
  ...
  return (
    <FormShell
      description="先补最小定义，后面再让系统围绕它安排学习或复习动作。"
      title="新增 Knowledge Point"
    >
      <label className="block space-y-2 text-sm text-[var(--xidea-charcoal)]">
        <span className="font-medium text-[var(--xidea-near-black)]">标题</span>
        <input
          className="w-full rounded-[0.95rem] border border-[var(--xidea-border)] bg-[var(--xidea-ivory)] px-3 py-2 outline-none focus:border-[var(--xidea-selection-border)]"
          onChange={(event) => onChange({ ...draft, title: event.target.value })}
          placeholder="例如：什么时候需要重排"
          value={draft.title}
        />
      </label>
      <label className="block space-y-2 text-sm text-[var(--xidea-charcoal)]">
        <span className="font-medium text-[var(--xidea-near-black)]">描述</span>
        <Textarea
          className="min-h-24 rounded-[0.95rem] border-[var(--xidea-border)] bg-[var(--xidea-ivory)] text-sm leading-7 text-[var(--xidea-charcoal)] focus-visible:ring-[var(--xidea-selection-border)]"
          onChange={(event) =>
            onChange({ ...draft, description: event.target.value })
          }
          placeholder="描述这个知识点要解决的判断、边界或设计取舍。"
          value={draft.description}
        />
      </label>
      <div className="space-y-2 text-sm text-[var(--xidea-charcoal)]">
        <span className="font-medium text-[var(--xidea-near-black)]">关联 Materials</span>
        {materials.length > 0 ? (
          <AssetPicker
            assets={materials}
            onToggle={(assetId) =>
              onChange({
                ...draft,
                sourceAssetIds: draft.sourceAssetIds.includes(assetId)
                  ? draft.sourceAssetIds.filter((id) => id !== assetId)
                  : [...draft.sourceAssetIds, assetId],
              })
            }
            selectedAssetIds={draft.sourceAssetIds}
          />
        ) : (
          <p className="text-sm text-[var(--xidea-stone)]">
            当前 project 还没有材料，先在新建 Project 时选材料，或后续从 session 挂接。
          </p>
        )}
      </div>
      <div className="flex flex-wrap gap-3">
...
export function ArchiveConfirmationCard({
  count,
  onCancel,
  onConfirm,
}: {
  ...
  return (
    <Card className="rounded-[1.35rem] border-[#ebd5cc] bg-[#f9efea] shadow-none">
      <CardContent className="flex flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm leading-6 text-[var(--xidea-charcoal)]">
            当前准备归档 {count} 个知识点。归档后它们会移到 Archived
            视图，不再作为默认学习池展示。
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
...
export function KnowledgePointBatchActions({
  filteredCount,
    ...
    <Card className="rounded-[1.35rem] border-[var(--xidea-border)] bg-[var(--xidea-white)] shadow-none">
      <CardContent className="flex flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm leading-6 text-[var(--xidea-charcoal)]">
            已选 {selectedCount} 个知识点。可以批量清空选择、全选当前筛选结果，或统一归档 / 恢复。
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <Button
          ...
          <Button
            ...
            variant="outline"
          >
            {hasArchivedSelection ? "恢复所选" : "归档所选"}
          </Button>
        </div>
      </CardContent>
    </Card>
