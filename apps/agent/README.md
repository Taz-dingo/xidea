...
兼容说明：`ZHIPU_API_KEY / ZAI_API_KEY / OPENAI_API_KEY` 仍可继续使用，但只作为旧环境回退读取来源，不再作为推荐配置口径。若只配置 `XIDEA_LLM_API_KEY + XIDEA_LLM_MODEL=glm-*`，运行时也会自动按智谱 OpenAI-compatible 路径发请求。
