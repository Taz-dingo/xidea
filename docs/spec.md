...
- `study / review session` 不再绑定单张知识卡；它们是围绕一个受控候选池运行的学习编排会话，用户首句会参与首轮选卡
...
5. 学习与复习会先进入待开始态；用户首句会参与首轮编排，系统再围绕 1 到 3 张主题相邻的知识卡组织一轮 session
6. session 内会显式展示“当前学习计划”：右侧显示当前有效计划，会话流只在首次编排或关键改排时插入专门的编排事件卡
7. 页面必须把“当前 project 在学什么、有哪些知识点、为什么这轮学这个、作答后如何回写状态”讲清楚
...
- 第一版 `study / review` 都采用“小候选池 + 当前学习计划”的 session 语义：点击入口先进入待开始态，用户首句参与首轮选卡，首次编排完成后才创建真实 session
- 第一版 session 右栏显示当前有效学习计划，会话流只显示首次编排和关键改排；前端不本地脑补编排语义
