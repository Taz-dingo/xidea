...
## 2026-04-16 — PR 描述默认用更详细的中文说明

### 决策

项目级 `pr-description` skill 默认产出更详细的中文 PR 描述，而不是只给极短的摘要。
在用户没有另行指定格式或语言时，PR 描述继续沿用：
...
- 这次改动为什么现在做
- 用户侧或 reviewer 需要理解的主要变化
- 实际验证了什么
- 还有哪些风险、依赖或后续工作

### 原因

- 进入并行开发阶段后，PR 描述不仅是合并说明，也是跨 owner 对齐上下文的主要入口
- 过短的 desc 容易让 reviewer 看不出共享边界、验证范围和风险，尤其是文档、contract 或跨层改动
- 默认中文说明更符合当前项目的日常协作语境

### 影响

- 用户要求“提 PR”或“写 PR 描述”时，`pr-description` 默认输出更完整的中文说明
- docs-only、contract 变更和跨 owner 改动需要在 PR 描述里显式交代共享影响
- 如果只是极短总结，视为不满足当前项目默认 PR desc 质量要求
