...
- `docs/reference/agent-state-design.md` 的 Tools 部分已重写
...
`docs/` 根目录只保留当前操作层文件，流程文档移动到 `docs/process/`，补充背景的参考文档移动到 `docs/reference/`，较早期的协作草案移动到 `docs/archive/`。

### 原因

- `docs/` 根目录平铺文件过多时，新进入仓库的人很难立刻区分什么是当前 source of truth
- 项目已经明确了 `spec / status / plan / memory` 这套 operating layer，目录结构应该直接把这个优先级表达出来
- 把流程层、参考层和归档层分开后，skills 与 README 也更容易给出稳定入口

### 影响

- 后续默认从 `docs/README.md` 进入文档结构，再根据任务进入 operating layer、process guides 或 reference docs
- `project-onboarding`、`docs-governance`、`branch-workflow` 等项目级 skills 需要跟随新路径维护
- `docs/archive/collaboration.md` 只作为历史草案保留，不再作为当前协作规则来源
